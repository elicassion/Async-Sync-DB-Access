		...
		async with create_pool(host=HOST, port=PORT,
			...
			async with pool.get() as conn:
				async with conn.cursor() as cur:
					await cur.execute("SELECT reviewerID, reviewerName, reviewTime \
										FROM review NATURAL JOIN reviewer \
...
def joinDbSync():
	logfile = open("{}_{}_join_log_s.csv".format(REPEATTIME, STEP), "w")
	count = 0
	...
	for i in range(REPEATTIME):
		try:
			cursor.execute("SELECT reviewerID, reviewerName, reviewTime \
							FROM review NATURAL JOIN reviewer \
							WHERE reviewTime LIKE '%{}';"
							.format(random.randint(2000, 2014)))
			# db.commit()
...
def joinDbAsync():
	logfile = open("{}_{}_join_log_a.csv".format(REPEATTIME, STEP), "w")
