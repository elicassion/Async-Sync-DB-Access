		...
		async with create_pool(host=HOST, port=PORT,
			...
			async with pool.get() as conn:
				async with conn.cursor() as cur:
					await cur.execute("UPDATE review SET helpful = '12345' \
										WHERE reviewerID = 'TESTUSER' \
										and reviewTime = {};"
...
def updDbSync():
	logfile = open("{}_{}_upd_log_s.csv".format(REPEATTIME, STEP), "w")
	count = 0
	...
	for i in range(REPEATTIME):
		try:
			cursor.execute("UPDATE review SET helpful = '12345' \
							WHERE reviewerID = 'TESTUSER' \
							and reviewTime = {};"
							.format(random.randint(2000, 2014)))
			db.commit()
...
def updDbAsync():
	logfile = open("{}_{}_upd_log_a.csv".format(REPEATTIME, STEP), "w")
